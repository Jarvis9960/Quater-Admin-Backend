...
const headerTwoHindi = require("../models/headerTwoSchema Hindi");
const { unlink } = require("fs");

const headerTwoControllerHindi = async (req, res) => {
  const headerHindiCount = await headerTwoHindi.count();
  ...
  try {
    if (headerHindiCount > 0) {
      ...
      const savedHeader = await headerTwoHindi.find({});
      ...
      const updatedHeader = await headerTwoHindi.updateOne(
        { _id: updateHeaderId },
      ...
      const newHeaderTwo = new headerTwoHindi({
        Title: title,
...
const getHeaderTwoControllerHindi = async (req, res) => {
  try {
    const savedResponse = await headerTwoHindi.find({});
...
module.exports = { headerTwoControllerHindi, getHeaderTwoControllerHindi };
